...
grid = {'batch_size': [10,20],'max_iter': [50,100,150],'eta' : [.001,.0001,.00001]}
m=grid_search(grid)
kf = KFold(n_splits=5)
accuracy_allfold=[]
biases_all=[]
...
#print "index ",index
with open("partb_weights.pkl","wb") as f:
    pickle.dump(weights_all[index],f)

with open("partb_biases.pkl","wb") as f:
    pickle.dump(biases_all[index],f)
with open("epoch_accuracy_partb.txt","w") as f:
